...
audio_input=${1:-./datasets/任务1-6/}
config_path=${CONFIG_PATH:-./config/config.yaml}
model_path=${MODEL_PATH:-}
hf_token=${HF_TOKEN:-}
hf_cache_dir=${HF_CACHE_DIR:-}
ref_path=${REF_RTTM:-${REF_RTTM_DIR:-./datasets/任务1-6/rttm/}}
debug_flag=${DEBUG:-1}
...
with open(path, 'r', encoding='utf-8') as f:
    for line in f:
        if '=' in line:
            ...
            values[k] = v
print(values.get('global_der') or values.get('avg_der', 'NA'))
PY
)
...
with open(path, 'r', encoding='utf-8') as f:
    for line in f:
        if '=' in line:
            k, v = line.strip().split('=', 1)
            values[k] = v
print(values.get('global_files') or values.get('files', '0'))
PY
)
            echo "$run_name -> DER(global): $der% over $files file(s)"
            echo "$run_name -> DER(global): $der% | files=$files" >> "$results_file"
        fi
    fi
else
